...
| 도구              | 버전      | 용도                              |
| ----------------- | --------- | --------------------------------- |
| Terraform         | v1.11.4   | 인프라 프로비저닝                 |
| AWS CLI           | 2.33.29   | AWS 리소스 접근 및 인증           |
| GNU Make          | -         | 스택별 init/plan/apply 래퍼       |
| terraform-docs    | -         | 모듈 문서 자동 생성               |
| tf-summarize      | -         | plan 결과 트리 형식 출력          |
| Atlantis          | -         | GitOps PR 기반 plan/apply 워크플로 (optional) |
...
### 1. Root (Environment) Stacks (`env/stg/apnortheast2/commerce/`)

> 실제 인프라 **배포 단위**이며, **global → network → ecr → elb → compute → cicd** 순서로 각 폴더의 Makefile을 통해 실행합니다.

| 스택 이름         | 경로                                       | 주요 설명                                                          |
| ----------------- | ------------------------------------------ | ------------------------------------------------------------------ |
| **global**        | `env/stg/apnortheast2/commerce/global`      | IAM, ACM, Route53 등 리전/계정 수준의 글로벌 및 공통 리소스        |
| **network**       | `env/stg/apnortheast2/commerce/network`     | VPC, Subnet, Route Table 등 기본 네트워크 인프라                   |
| **ecr**           | `env/stg/apnortheast2/commerce/ecr`         | 이미지 보관용 ECR Container Registry                               |
| **elb**           | `env/stg/apnortheast2/commerce/elb`         | Application/Network Load Balancer 및 Target Group                  |
| **compute (EC2)** | `env/stg/apnortheast2/commerce/compute/ec2` | EC2 인스턴스 기반 컴퓨팅 리소스                                    |
| **compute (ECS)** | `env/stg/apnortheast2/commerce/compute/ecs` | ECS 클러스터 및 Fargate/EC2 워크로드                               |
| **cicd**          | `env/stg/apnortheast2/commerce/cicd`        | CodeDeploy를 포함한 CI/CD 배포 파이프라인 구성 리소스              |
| **storage**       | `env/stg/apnortheast2/commerce/storage`     | S3 버킷 생성 및 정책/라이프사이클 규칙 관리                        |
| **\_common**      | `env/stg/apnortheast2/commerce/_common`     | 환경 내 스택들이 공통으로 참조하는 Provider 및 Backend 설정 가이드 |
...
cd env/stg/apnortheast2/commerce/network
